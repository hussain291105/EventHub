...
const testBookingFlow = async () => {
  try {
    ...
    // 2. Get ticket types
    const ticketTypesResponse = await fetch(`http://localhost:5000/api/events/${firstEvent.id}/ticket-types`);
    ...
    const confirmResponse = await fetch('http://localhost:5000/api/confirm-mock-payment', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        paymentIntentId,
    ...
    const debugResponse = await fetch('http://localhost:5000/api/debug/bookings');
